...
export default function PostSupportDetailsPage() {
    ...
    if (loading) {
        return (
            <Layout>
            <PageHeader
                title={locale === 'tr' ? 'Post Desteği' : 'Post Support'}
                breadcrumb={locale === 'tr' ? 'Yükleniyor...' : 'Loading...'}
                slug="post-destegi"
                py={10}
            />
    ...
    if (!data) {
        return (
            ...
            <PageHeader
                title={locale === 'tr' ? 'Post Desteği' : 'Post Support'}
                breadcrumb={locale === 'tr' ? 'Hata' : 'Error'}
                slug="post-destegi"
                py={10}
            />
    ...
    return (
        <Layout>
            <PageHeader
                title={`${title} ${locale === 'tr' ? 'Post Desteği' : 'Post Support'}`}
                breadcrumb={`${title} ${locale === 'tr' ? 'Post Desteği' : 'Post Support'}`}
                slug="post-destegi"
                py={10}
            />
